...
import { User as UserIcon, CreditCard, Receipt, Settings, Shield, Check, Download, AlertCircle, Globe, Lock, Upload, X } from 'lucide-react';
...
export const AccountSettings: React.FC<AccountSettingsProps> = ({ user, onUpdateUser }) => {
  ...
  const confirmUpgrade = () => {
    ...
    const planPrice = pendingTier === 'PREMIUM_2K' ? 24.99 : 34.99;
          ...
          {activeTab === 'subscription' && (
             <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {[
                     { id: 'FREE', name: 'Free Tier', price: 0, features: ['Standard Quality', 'Web Resolution', 'Basic Styles'] },
                     { id: 'PREMIUM_2K', name: 'Premium 2K', price: 24.99, features: ['2K Resolution', 'High Quality', 'Priority Rendering', 'Commercial Use'] },
                     { id: 'ULTRA_4K', name: 'Ultra 4K', price: 34.99, features: ['4K Ultra HD', 'Photorealistic', 'Agency Tools', 'API Access'] }
                  ].map((plan) => {
                    ...
                    >
                       {user.tier === plan.id && (
                         <div className="absolute top-0 right-0 bg-yellow-500 text-black text-[10px] font-bold px-2 py-1 rounded-bl-lg rounded-tr-lg">
                           CURRENT
                         </div>
                       )}
                       <div className="mb-4">
              ...
              <div className="p-8">
                ...
                <div className="bg-zinc-950 rounded-lg p-4 space-y-3 mb-6 border border-zinc-800">
                   <div className="flex justify-between text-sm">
                      ...
                      <span className="text-white font-medium">€{(pendingTier === 'PREMIUM_2K' ? 24.99 : 34.99).toFixed(2)}</span>
                   </div>
                   <div className="flex justify-between text-sm">
                      <span className="text-zinc-400">VAT ({currentVatRate}% - {country})</span>
                      <span className="text-white font-medium">€{calculateTotal(pendingTier === 'PREMIUM_2K' ? 24.99 : 34.99).vat.toFixed(2)}</span>
                   </div>
                   <div className="border-t border-zinc-800 pt-3 flex justify-between items-center">
                      <span className="font-bold text-white">Total</span>
                      <span className="font-bold text-xl text-yellow-500">€{calculateTotal(pendingTier === 'PREMIUM_2K' ? 24.99 : 34.99).total.toFixed(2)}</span>
                   </div>
                </div>
